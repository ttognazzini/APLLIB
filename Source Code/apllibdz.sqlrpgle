...
// Fabricut Lbiraries Filter

Dcl-F APLLIBFZ workstn infds(dspDs) UsrOpn;

/Copy QSRC,BASFNCV1PR // prototypes for all #$ procedures
/Copy QSRC,APLSRVV1PR // prototypes for $ procedures used for template programs
/Copy QSRC,APLLIBDZ_@ // auto generatde data structures for field attribute fields
/Copy QSRC,APLLIBDZPR // Always include the prototype for the current program
/Copy QSRC,DCTVALBPPR
/Copy QSRC,DCTVALDPPR

Dcl-Ds filterDs extname('APLLIBFZ') Inz End-DS;
...
Dcl-Proc Prompt;
  ...
  // Prompt Library Type Description
  If CsrFld = 'LIBTYPD';
...
Dcl-Proc ValidateScreen;
  ...
  libdes   =%scanrpl('%':'':libdes);
  libTypD=%scanrpl('%':'':libTypD);
  ...
  // make sure the library type is correct, Auto prompt first if not valid, handles ?
  If libTypD<>'';
    pmEnmVal='';
    pmEnmDes=libTypD;
    ...
    If libTypD <> pmEnmDes;
      ...
      error=*on;
    EndIf;
    If not $ValidEnmDes(#$UPIFY(libTypD):'APLDCT':'libTyp');
