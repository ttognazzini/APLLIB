...
Dcl-F FLELOGF1 WorkStn SFile(SFL:RRN1) InfDS(dspDs) UsrOpn;
...
Dcl-S fldNmeFltSav Like(fldNme); // used to see if the field name filter changed, to stop screen progression
...
// This is done to override the one in XXXXXX_@ so the CRTDTM column can be descending
// **Change this must now be manually updated
Dcl-Ds orderByDS2 Qualified;
  values char(160)
    inz('CRTDTM desc         +
         CRTUSR              +
         FLDNME              +
         ...
         LOGDES              +
          ');
  Value char(20) dim(8) overlay(values);
End-Ds;
...
Dcl-Proc Prompt;
  ...
  If CsrFld = 'FLDNMEFLT';
    pmOption = '1';
    Callp FLEFLDD1(fleLib:fleNme:pmFldNme:pmOption:keyPressed);
    If keyPressed = 'ENTER';
      fldNmeFlt = pmFldNme;
      LoadSFL();
    EndIf;
...
Dcl-Proc ProcessLine;
  ...
  ElseIf Sel<>'';
    FLELOGB0(fleLogIdn:Sel:keyPressed);
...
Dcl-Proc BuildSQLStatement;
  ...
  orderBy = 'Order By ' + %trim(orderByDS2.value(filterDs.srtCde))+',key';
  ...
  // Filter on field name if entered
  If fldNmeFlt > '';
    sqlStm += ' and upper(fldNme) = ''' + %upper(fldNmeFlt) + '''';
  EndIf;
...
Dcl-Proc ProgramInitialization;
  ...
  If not %open(FLELOGF1); // **change
    Open FLELOGF1; // **change
  EndIf;
  ...
  colTxt = '';
  If fldNmeFlt <> '';
    Exec SQL Select colTxt into :colTxt from DCTFLD where (dctNme,fldNme) = (:DctNme,:fldNmeFlt);
  EndIf;
