...
// Field Type Master List

Dcl-F FLDTYPF1 WorkStn SFile(SFL:RRN1) InfDS(dspDs) UsrOpn;

/Copy QSRC,BASFNCV1PR // prototypes for all #$ procedures
/Copy QSRC,APLSRVV1PR // prototypes for $ procedures used for template programs
/Copy QSRC,FLDTYPD1_@ // auto generated data structures for field attribute fields
/Copy QSRC,FLDTYPD1PR // Always include the prototype for the current program
/Copy QSRC,FLDTYPDZPR
/Copy QSRC,FLDTYPB0PR
...
Dcl-Ds filterDs ExtName('FLDTYPFZ') Qualified Inz End-Ds; // contains the filter and sort options from DZ

Dcl-S Option  like(APLDCT.Option); // global variable to store the option parameter in
Dcl-S fldTypIdn like(APLDCT.fldTypIdn);
...
Dcl-S selected  Ind;
Dcl-S selFldTyp Like(APLDCT.FldTyp);
...
Dcl-Proc Filter;
  ...
  Callp FLDTYPDZ(filterDs:keyPressed);
...
Dcl-Proc UpdateScreen;
  ...
  Else;
    ...
    DoW optionsCount>0;
      Clear sflFields;
      fldTypIdn  = optionsArray(1).key;
      Sel  = optionsArray(1).Option;
      positionToKey=optionsArray(1).key;
      Exec SQL select fldTyp into :fldTyp from FLDTYP where fldTypIdn = :fldTypIdn;
      ProcessLine();
...
Dcl-Proc ProcessLine;
  ...
  ElseIf Sel<>'';
    FLDTYPB0(fldTyp:Sel:keyPressed);
...
Dcl-Proc Add;
  ...
  FLDTYPB0(pmFldTyp:pmOption:keyPressed);
...
Dcl-Proc ValidateScreen;
  ...
  If Sel1<>'';
    ...
    ElseIf not #$IN(Sel:'8':'1') and (fldTyp = '');
      $ErrorMessage('DCT1001':'':error:fldTyp1@:'fldTyp1':outRow:outCol:psDsPgmNam);

      // If deleting make sure the row is active
    ElseIf Sel = '4' and tsAcvRow<>'1';
      $ErrorMessage('DCT1004':'':error:fldTyp1@:'fldTyp1':outRow:outCol:psDsPgmNam);

      // If reinstating make sure the row is inactive
    ElseIf Sel = '13' and tsAcvRow<>'0';
      $ErrorMessage('DCT1013':'':error:Sel1@:'Sel1':outRow:outCol:psDsPgmNam:fldTyp1@);

      // If adding make sure the entry does not exist
    ElseIf Sel = '1' and not Maintenance and found;
      $ErrorMessage('DCT1101':'':error:fldTyp1@:'fldTyp1':outRow:outCol:psDsPgmNam);

      // If any option but add or position to, make sure the entry exists
    ElseIf not #$IN(Sel:'1':'8') and not found and not Maintenance;
      $ErrorMessage('DCT1102':'':error:fldTyp1@:'fldTyp1':outRow:outCol:psDsPgmNam);
...
Dcl-Proc BuildSQLStatement;
  ...
  sqlStm='+
    ...
    from ( +
      ...
      from FLDTYP +
      left join DCTVAL as acv on (acv.DctNme,acv.FldNme,acv.EnmVal) = (''APLDCT'',''ACVROW'',FLDTYP.acvRow)     +
    )';
  ...
  If filterDs.fldTyp > '';
    sqlStm += ' ' + Where + ' '
      + ' ucase(fldTyp) = uCase(''' + %trim(filterDs.fldTyp) + ''')';
    Where = 'and';
  EndIf;
...
Dcl-Proc ProgramInitialization;
  ...
  Exec SQL
       Declare FLDTYP Asensitive Scroll Cursor For DYNSQLStm;

  // Open the screen DSPF if it is not already open
  If not %open(FLDTYPF1); // **change
    Open FLDTYPF1; // **change
  EndIf;
  ...
  ElseIf Option = '1';
    $GetFieldLocation(PgmNme:'sel':outRow:outCol);
  Else;
    $GetFieldLocation(PgmNme:'fldTyp1':outRow:outCol);
  EndIf;
