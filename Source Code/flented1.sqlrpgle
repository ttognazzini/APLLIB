...
// File Notes Maintenance

Dcl-F FLENTEF1 WorkStn SFile(SFL:RRN1) InfDS(dspDs) UsrOpn;

/Copy QSRC,BASFNCV1PR // prototypes for all #$ procedures
/Copy QSRC,APLSRVV1PR // prototypes for $ procedures used for template programs
/Copy QSRC,FLENTED1_@ // auto generated data structures for field attribute fields
/Copy QSRC,FLENTED1PR // Always include the prototype for the current program
...
Dcl-Proc UpdateScreen;
  ...
  Exec SQL
    Insert Into FLENTE
      ...
      Select :fleLib,:fleNme, nteSeq,:fleMstIdn, nte,
  ...
  Exec SQl Select listagg(trim(nte),' ') within group(order by nteSeq)
            ...
            from FLENTE
            where (fleLib,fleNme) = (:fleLib,:fleNme);
  sqlStm = 'Comment on table ' + %trim(fleLib) + '/' + %trim(fleNme) + ' is ''' + #$dblq(notes) + '''';
  ...
  Exec SQL
    ...
    Set nteExs =
      Case when (
          Select count(*)
          From FLENTE
          Where (FLENTE.fleLib,FLENTE.FleNme)
              = (:fleLib,:fleNme)
        ) >= 1 then 'Y'
      Else 'N' end
    Where (FLEMST.fleLib,FLEMST.FleNme)
        = (:fleLib,:fleNme);
...
Dcl-Proc ProgramInitialization;
  ...
  // get the file name from the master file
