...
Dcl-F OTQMSTF1 WorkStn SFile(SFL:RRN1) InfDS(dspDs) UsrOpn;

/Copy QSRC,BASFNCV1PR // prototypes for all #$ procedures
/Copy QSRC,APLSRVV1PR // prototypes for $ procedures used for template programs
/Copy QSRC,OTQMSTD1_@ // auto generated data structures for field attribute fields
/Copy QSRC,OTQMSTD1PR // Always include the prototype for the current program
/Copy QSRC,OTQMSTDZPR
/Copy QSRC,OTQMSTB0PR
...
Dcl-Ds filterDs ExtName('OTQMSTFZ') Qualified Inz End-Ds; // contains the filter and sort options from DZ
...
Dcl-S selected  Ind;
Dcl-S selotqNme Like(APLDCT.otqNme);
...
Dcl-Proc Filter;
  ...
  Callp OTQMSTDZ(filterDs:keyPressed);
...
Dcl-Proc UpdateScreen;
  ...
  If Sel1 <> '' or line1<>line1Defaults;
    ...
    EndIf;
    Clear OTQMSTIdn;
    Exec SQL Select OTQMSTIdn into :OTQMSTIdn
             From OTQMST
             Where otqNme = :otqNme;
    ProcessLine();
    Clear Sel1;
...
Dcl-Proc ProcessLine;
  ...
  ElseIf Sel<>'';
    OTQMSTB0(OTQMSTIdn:Sel:keyPressed);
...
Dcl-Proc Add;
  ...
  OTQMSTB0(pmIdn:pmOption:keyPressed);
...
Dcl-Proc ValidateScreen;
  ...
  If Sel1<>'';
    ...
    ElseIf not (sel in %list('8':'1')) and otqNme = '';
      $ErrorMessage('DCT1001':'':error:otqNme1@:'otqNme1':outRow:outCol:psDsPgmNam);

      // If deleting make sure the row is active
    ElseIf Sel = '4' and tsAcvRow<>'1';
      $ErrorMessage('DCT1004':'':error:otqNme1@:'otqNme1':outRow:outCol:psDsPgmNam);

      // If reinstating make sure the row is inactive
    ElseIf Sel = '13' and tsAcvRow<>'0';
      $ErrorMessage('DCT1013':'':error:Sel1@:'Sel1':outRow:outCol:psDsPgmNam:otqNme1@);

      // If adding make sure the entry does not exist
    ElseIf Sel = '1' and not Maintenance and found;
      $ErrorMessage('DCT1101':'':error:otqNme1@:'otqNme1':outRow:outCol:psDsPgmNam);
    ...
    ElseIf not (Sel in %list('1':'8')) and not found and not Maintenance;
      $ErrorMessage('DCT1102':'':error:otqNme1@:'otqNme1':outRow:outCol:psDsPgmNam);
    EndIf;
  EndIf;
  ...
  For sflRcd = 1 To RRN1SV;
    ...
    If %found;
      ...
      ElseIf Sel = '2' and wtrSts <> 'STR';
        $ErrorMessage(' ':'Writer is not started.':error:
        sel@:'Sel':outRow:outCol:psDsPgmNam);
        outRow+=sflRcd-1; // required to move to the correct SFL line
      ElseIf Sel = '4' and acvDes = 'Inactive';
        $ErrorMessage('DCT1004':'':error:sel@:'Sel':outRow:outCol:psDsPgmNam);
        outRow+=sflRcd-1; // required to move to the correct SFL line
      ElseIf Sel = '9' and wtrSts = 'STR';
        $ErrorMessage(' ':'Write is already started.':error:
        sel@:'Sel':outRow:outCol:psDsPgmNam);
        outRow+=sflRcd-1; // required to move to the correct SFL line
      ElseIf Sel = '12' and wtrSts <> 'STR';
        $ErrorMessage(' ':'Writer is not started.':error:
        sel@:'Sel':outRow:outCol:psDsPgmNam);
        outRow+=sflRcd-1; // required to move to the correct SFL line
      EndIf;
...
Dcl-Proc LoadSFL;
  ...
  For i = 1 To SFLPage;
    If dta(i).otqNme<>'';
      ...
      // check the formtype of the printer
      pmrWtrNme = otqNme;
      pmrWtrSts = ' ';
      pmrFrmTyp = ' ';
...
Dcl-Proc BuildSQLStatement;
  ...
  sqlStm=' +
    ...
    From ( +
      Select Distinct +
        OTQMST.OTQLIB otqLib, +
        OTQMST.OTQMSTIdn Key, +
        OTQMST.OTQNME otqNme, +
        Coalesce(substr(text,1,40),'' '') des, +
        ''          '' wtrsts, +
        ''          '' frmtyp, +
        OTQMST.PCYNBR pcyNbr, +
        OTQMST.PRTTYP prtTyp +
      From OTQMST +
      Left Join OUTPUT_QUEUE_INFO on OTQMST.OTQLIB = outqlib AND +
                                    OTQMST.OTQNME = outq +
      Where OTQMST.PRTTYP = ''ZEBRA'' +
    )';
  ...
  // If filterDs.otqNme > '';
  //   sqlStm += ' ' + Where + ' '
  //     + ' ucase(otqNme) Like uCase(''%' + %trim(filterDs.otqNme) + '%'')';
  //   Where = 'and';
  // EndIf;
...
Dcl-Proc ProgramInitialization;
  ...
  Exec SQL
       Declare OTQMST Asensitive Scroll Cursor For DYNSQLStm;

  // Open the screen DSPF if it is not already open
  If not %open(OTQMSTF1); // **change
    Open OTQMSTF1; // **change
  EndIf;
  ...
  ElseIf Option = '1';
    $GetFieldLocation(PgmNme:'sel':outRow:outCol);
  Else;
    $GetFieldLocation(PgmNme:'otqNme1':outRow:outCol);
  EndIf;
