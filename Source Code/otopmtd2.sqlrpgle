...
Dcl-F OTOPMTF2 WORKSTN InfDS(dspDs) UsrOpn;

/Copy QSRC,BASFNCV1PR // prototypes for all #$ procedures
/Copy QSRC,APLSRVV1PR // prototypes for $ procedures used for template programs
/Copy QSRC,OTOSRVV1PR // prototypes for output options procedures
/Copy QSRC,OTOPMTD2_@ // auto generatde data structures for field attribute fields
/Copy QSRC,PMPWDWD1PR // Generic prompt window
...
Dcl-Proc ValidateScreen;
  ...
  // Validate from Email addresses
  If #$VEML(frmEml);
    $ErrorMessage('':'Error invalid from email address.':error:frmEml@:'frmEml':outRow:outCol:psDsPgmNam);
  EndIf;
  If #$VEML(emlAdd);
    $ErrorMessage('':'Error invalid tom email address.':error:emlAdd@:'emlAdd':outRow:outCol:psDsPgmNam);
  EndIf;
  If cc1Eml <> '' and #$VEML(cc1Eml);
    $ErrorMessage('':'Error invalid from CC 1 email address.':error:cc1Eml@:'cc1Eml':outRow:outCol:psDsPgmNam);
  EndIf;
  If cc2Eml <> '' and #$VEML(cc2Eml);
    $ErrorMessage('':'Error invalid from CC 2 email address.':error:cc2Eml@:'cc2Eml':outRow:outCol:psDsPgmNam);
  EndIf;
  If cc3Eml <> '' and #$VEML(cc3Eml);
    $ErrorMessage('':'Error invalid from CC 3 email address.':error:cc3Eml@:'cc3Eml':outRow:outCol:psDsPgmNam);
  EndIf;
  If bccEml <> '' and #$VEML(bccEml);
    $ErrorMessage('':'Error invalid from BCC email address.':error:bccEml@:'bccEml':outRow:outCol:psDsPgmNam);
  EndIf;
  ...
  // if there are no other erross and no email subject, give a warning
  If not error and emlSbj= '';
    PMPWDWD1('Email does not have a subject, send anyway?':ans:'YN':'Y=Yes, N=No':keyPressed);
    If keyPressed='F3' or keyPressed='F12';
      error = *on; // redisplay screen
    ElseIf ans='Y';
      // just send the email
    ElseIf ans='N';
      $ErrorMessage('':'Enter email subject.':error:emlSbj@:'emlSbj':outRow:outCol:psDsPgmNam);
    EndIf;
  EndIf;
