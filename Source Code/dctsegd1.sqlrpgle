...
Dcl-F DCTSEGF1 WorkStn SFile(SFL:RRN1) InfDS(dspDs) UsrOpn;

/Copy QSRC,BASFNCV1PR // prototypes for all #$ procedures
/Copy QSRC,APLSRVV1PR // prototypes for $ procedures used for template programs
/Copy QSRC,DCTSEGD1_@ // auto generated data structures for field attribute fields
/Copy QSRC,DCTSEGD1PR // Always include the prototype for the current program
/Copy QSRC,DCTSEGDZPR
/Copy QSRC,DCTSEGB0PR
...
Dcl-Ds filterDs ExtName('DCTSEGFZ') Qualified Inz End-Ds; // contains the filter and sort options from DZ
...
Dcl-S selected  Ind;
Dcl-S seldtaSeg Like(APLDCT.dtaSeg);
...
Dcl-Proc Filter;
  ...
  Callp DCTSEGDZ(filterDs:keyPressed);
...
Dcl-Proc ProcessLine;
  ...
  ElseIf Sel<>'';
    DCTSEGB0(dtaSeg:Sel:keyPressed);
...
Dcl-Proc Add;
  ...
  DCTSEGB0(pmDtaSeg:pmOption:keyPressed);
...
Dcl-Proc ValidateScreen;
  ...
  If Sel1<>'';
    ...
    ElseIf not #$IN(Sel:'8':'1') and dtaSeg = '';
      $ErrorMessage('DCT1001':'':error:dtaSeg1@:'dtaSeg1':outRow:outCol:psDsPgmNam);

      // If deleting make sure the row is active
    ElseIf Sel = '4' and tsAcvRow<>'1';
      $ErrorMessage('DCT1004':'':error:dtaSeg1@:'dtaSeg1':outRow:outCol:psDsPgmNam);

      // If reinstating make sure the row is inactive
    ElseIf Sel = '13' and tsAcvRow<>'0';
      $ErrorMessage('DCT1013':'':error:Sel1@:'Sel1':outRow:outCol:psDsPgmNam:dtaSeg1@);

      // If adding make sure the entry does not exist
    ElseIf Sel = '1' and not Maintenance and found;
      $ErrorMessage('DCT1101':'':error:dtaSeg1@:'dtaSeg1':outRow:outCol:psDsPgmNam);

      // If any option but add or position to, make sure the entry exists
    ElseIf not #$IN(Sel:'1':'8') and not found and not Maintenance;
      $ErrorMessage('DCT1102':'':error:dtaSeg1@:'dtaSeg1':outRow:outCol:psDsPgmNam);
    EndIf;
  EndIf;
...
Dcl-Proc BuildSQLStatement;
  ...
  sqlStm = '+
    ...
    Select +
      ...
      dtaSeg, +
      des, +
      coalesce(a.EnmDes,''Error'') acvDes, +
      coalesce(b.EnmDes,''No'') cnfExsD +
    From DCTSEG +
    left join DCTVAL as a on (a.DctNme,a.FldNme,a.EnmVal)=(''APLDCT'',''ACVROW'',DCTSEG.AcvRow) +
    left join DCTVAL as b on (b.DctNme,b.FldNme,b.EnmVal)=(''APLDCT'',''CNFEXS'',DCTSEG.CnfExs) +
    )';
  ...
  // Conflict Exists
  If #$UPIFY(filterDs.CnfExsD) <> '';
    sqlStm += ' ' + Where + ' ucase(CnfExsD) = ''' + #$UPIFY(filterDs.CnfExsD) +'''';
    Where = 'and';
  EndIf;
  ...
  If filterDs.dtaSeg > '';
    sqlStm += ' ' + Where + ' '
      + ' ucase(dtaSeg) Like uCase(''%' + %trim(filterDs.dtaSeg) + '%'')';
    Where = 'and';
  EndIf;
...
Dcl-Proc ProgramInitialization;
  ...
  If not %open(DCTSEGF1); // **change
    Open DCTSEGF1; // **change
  EndIf;
  ...
  ElseIf Option = '1';
    $GetFieldLocation(PgmNme:'sel':outRow:outCol);
  Else;
    $GetFieldLocation(PgmNme:'dtaSeg1':outRow:outCol);
  EndIf;
