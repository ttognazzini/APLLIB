...
Dcl-F FLEMSTF1 WorkStn SFile(SFL:RRN1) InfDS(dspDs) UsrOpn;

/Copy QSRC,BASFNCV1PR // prototypes for all #$ procedures
/Copy QSRC,APLSRVV1PR // prototypes for $ procedures used for template programs
/Copy QSRC,FLEMSTD1_@ // auto generated data structures for field attribute fields
/Copy QSRC,FLEMSTD1PR // Always include the prototype for the current program
/Copy QSRC,FLEMSTDZPR
/Copy QSRC,FLEMSTB0PR
...
Dcl-Ds filterDs ExtName('FLEMSTFZ') Qualified Inz End-Ds; // contains the filter and sort options from DZ
...
Dcl-S selected  Ind;
Dcl-S selFleLib Like(APLDCT.FleLib);
...
Dcl-Proc Filter;
  ...
  Callp FLEMSTDZ(filterDs:keyPressed);
...
Dcl-Proc UpdateScreen;
  ...
  Else;
    ...
    DoW optionsCount>0;
      Clear sflFields;
      fleMstIdn = optionsArray(1).key;
      Exec SQL Select flelib,fleNme into :fleLib,:fleNme from FLEMST where fleMstIdn = :fleMstIdn;
...
Dcl-Proc ProcessLine;
  ...
  ElseIf Sel<>'';
    FLEMSTB0(fleLib:fleNme:Sel:keyPressed);
...
// Process F6=Add
Dcl-Proc Add;
  Dcl-S pmFleLib like(APLDCT.fleLib);
  Dcl-S pmFleNme like(APLDCT.fleNme);
  Dcl-S pmOption like(APLDCT.Option);
  pmOption='1';
  pmFleLib='';
  pmFleNme='';

  FLEMSTB0(pmFleLib:pmFleNme:pmOption:keyPressed);
...
Dcl-Proc ValidateScreen;
  ...
  If Sel1<>'';
    ...
    If Sel<>'' and not $ValidSFLOption(sel:optDs);
      ...
      // for any option but add and position to, the library and file names must be provided
    ElseIf not #$IN(Sel:'8':'1') and (fleNme = '' or fleLib = '');
      $ErrorMessage('DCT1001':'':error:fleNme1@:'fleNme1':outRow:outCol:psDsPgmNam);

      // If deleting make sure the row is active
    ElseIf Sel = '4' and tsAcvRow<>'1';
      $ErrorMessage('DCT1004':'':error:fleNme1@:'fleNme1':outRow:outCol:psDsPgmNam);

      // If reinstating make sure the row is inactive
    ElseIf Sel = '13' and tsAcvRow<>'0';
      $ErrorMessage('DCT1013':'':error:Sel1@:'Sel1':outRow:outCol:psDsPgmNam:fleNme1@);

      // If adding make sure the entry does not exist
    ElseIf Sel = '1' and not Maintenance and found;
      $ErrorMessage('DCT1101':'':error:fleNme1@:'fleNme1':outRow:outCol:psDsPgmNam);

      // If any option but add or position to, make sure the entry exists
    ElseIf not #$IN(Sel:'1':'8') and not found and not Maintenance;
      $ErrorMessage('DCT1102':'':error:fleNme1@:'fleNme1':outRow:outCol:psDsPgmNam);

      // make sure DDS type files are not updated here
    ElseIf sel in %list('2':'3':'4':'7':'13') and TblTyp = 'File';
      $ErrorMessage('':'Error - files must be updated via DDS, not here.'
                      :error:Sel1@:'Sel1':outRow:outCol:psDsPgmNam);

      // make sure views type files are not updated here
    ElseIf sel in %list('2':'3':'4':'7':'13') and TblTyp = 'View';
      $ErrorMessage('':'Error - views must be updated via source code, not here.'
                      :error:Sel1@:'Sel1':outRow:outCol:psDsPgmNam);

      // do not allow a hard deleted for production files
    ElseIf sel = '16';
      Exec SQL Select prdFle into :prdFle from FLEMST where (fleLib,fleNme) = (:fleLib,:fleNme);
      If prdFle = 'Y';
        $ErrorMessage('':'Error - a production file cannot be hard deleted, DeActivate it instead.'
                        :error:Sel1@:'Sel1':outRow:outCol:psDsPgmNam);
      EndIf;
    EndIf;
...
Dcl-Proc LoadSFL;
  ...
  For i = 1 To SFLPage;
    If dta(i).fleNme<>'';
      ...
      If positionToKey=dta(i).key and dta(i).key <> 0;
...
Dcl-Proc BuildSQLStatement;
  ...
  sqlStm='+
    ...
    From ( +
      Select Distinct +
        ...
        fleLib, +
        fleNme, +
        substr(fleDes,1,50) fleDes, +
        ''Data'' tblTyp, +
        substr(coalesce(Sts.EnmDes,''Production''),1,11) stsDes, +
        fldCnt fldCnt, +
        idxCnt idxCnt, +
        case when nteExs = ''Y'' then ''Y'' else '''' end nteExs, +
        FLEMST.dctNme, +
        case when fleErr = ''Y'' then ''Y'' else '''' end fleErr +
      from FLEMST +
      left join DCTVAL as Sts +
        on (Sts.DctNme,Sts.fldNme,Sts.enmVal) = (''APLDCT'',''FLESTS'',coalesce(FLEMST.fleSts,'''')) +
    )';
  ...
  // AcvRow
  If filterDs.stsDes <> '';
    sqlStm += ' ' + Where + ' uCase(stsDes) = Ucase(''' + %trim(filterDs.stsDes) + ''')';
    Where = 'and';
  EndIf;
  ...
  If filterDs.fleLib > '';
    sqlStm += ' ' + Where + ' '
      + ' ucase(fleLib) = uCase(''' + %trim(filterDs.fleLib) + ''')';
    Where = 'and';
  EndIf;

  If filterDs.fleNme > '';
    sqlStm += ' ' + Where + ' '
      + ' ucase(fleNme) = uCase(''' + %trim(filterDs.fleNme) + ''')';
    Where = 'and';
  EndIf;

  If filterDs.tblTyp > '';
    sqlStm += ' ' + Where + ' '
      + ' ucase(tblTyp) = uCase(''' + %trim(filterDs.tblTyp) + ''')';
    Where = 'and';
  EndIf;
  ...
  If filterDs.fleDes > '';
    sqlStm += ' ' + Where + ' '
      + ' ucase(fleDes) Like uCase(''%' + %trim(filterDs.fleDes) + '%'')';
    Where = 'and';
  EndIf;
...
Dcl-Proc ProgramInitialization;
  ...
  If not %open(FLEMSTF1); // **change
    Open FLEMSTF1; // **change
  EndIf;
