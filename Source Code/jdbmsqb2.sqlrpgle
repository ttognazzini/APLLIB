...
/copy QSRC,JDBMSQB1PR // Set up Java environment for Postgress, also contains postgress connection stuff
...
Dcl-Proc Main;
  ...
  If not connectDB();
    #$SNDMSG('Error - Could not connect to server':'*ESCAPE');
    Return;
  EndIf;
...
Dcl-Proc CreateTable;
  ...
  sqlStm = 'Drop Table JDBMSQB2';
  If JDBC_ExecUpd( conn : sqlStm ) < 0;
    #$SNDMSG('Table JDBMSQB2 not deleted.');
  Else;
    #$SNDMSG('Table JDBMSQB2 deleted.');
  EndIf;

  // Create the table
  sqlStm = '+
    create table JDBMSQB2 ( +
...
Dcl-Proc insertName;
  ...
  // Insert some rows
  sqlStm = 'insert into JDBMSQB2 (name) values(''' + #$DBLQ(name) + ''')';
  If JDBC_ExecUpd( conn : sqlStm ) < 0;
    #$SNDMSG('Error - Could not insert name '+ name +', sqlStm: "'+ sqlStm+ '".':MSGTYP);
  Else;
    #$SNDMSG('Name ' + name + ' added to table JDBMSQB2.');
  EndIf;
...
Dcl-Proc ReadRows;
  ...
  sqlStm = 'Select id, active, name From JDBMSQB2 order by id';
