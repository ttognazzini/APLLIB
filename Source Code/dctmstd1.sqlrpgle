...
Dcl-F DCTMSTF1 WorkStn SFile(SFL:RRN1) InfDS(dspDs) UsrOpn;

/Copy QSRC,BASFNCV1PR // prototypes for all #$ procedures
/Copy QSRC,APLSRVV1PR // prototypes for $ procedures used for template programs
/Copy QSRC,DCTMSTD1_@ // auto generated data structures for field attribute fields
/Copy QSRC,DCTMSTD1PR // Always include the prototype for the current program
/Copy QSRC,DCTMSTDZPR // Dictionary master - filters
/Copy QSRC,DCTMSTB0PR // Dictionary master - navigator
/Copy QSRC,DCTMSTBRPR // Dictionary master - print list driver
...
Dcl-Proc DisplayScreen;
  ...
  // Set the text for F11=Show/Hide Deleted
  If filterDs.acvDes = '';
    fncKeys=$ChangeFunctionKey(fncDs:'F11':'F11=Hide Inactive');
  Else;
    fncKeys=$ChangeFunctionKey(fncDs:'F11':'F11=Show Inactive');
  EndIf;
...
// Toggle between showing and hiding inactive
Dcl-Proc ToggleInactive;
...
Dcl-Proc UpdateScreen;
  ...
  If Sel1 <> '' or line1<>line1Defaults;
    ...
    EndIf;
    Clear dctMstIdn;
    Exec SQL Select dctMstIdn into :dctMstIdn
             From DCTMST
             Where dctNme = :dctNme;
    ProcessLine();
    Clear Sel1;
...
Dcl-Proc ProcessLine;
  ...
  ElseIf Sel <> '';
    DCTMSTB0(dctMstIdn:Sel:keyPressed);
...
Dcl-Proc Add;
  ...
  DCTMSTB0(pmIdn:pmOption:keyPressed);
...
Dcl-Proc ValidateScreen;
  ...
  If Sel1 <> '';
    ...
    If Sel <> '' and not $ValidSFLOption(sel:optDs);
      $ErrorMessage('Invalid Option.':'':error:Sel@:'Sel':outRow:outCol:psDsPgmNam);

      // for any option but add and position to, the dictionary name must be provided
    ElseIf not (sel in %list('8':'1')) and DctNme = '';
      $ErrorMessage('':'Dictionary name is requried':error:DctNme1@:'DctNme1':outRow:outCol:psDsPgmNam);

      // If deleting make sure the row is active
    ElseIf Sel = '4' and tsAcvRow <> '1';
      $ErrorMessage('':'Row is already inactive.':error:DctNme1@:'DctNme1':outRow:outCol:psDsPgmNam);

      // If reinstating make sure the row is inactive
    ElseIf Sel = '13' and tsAcvRow <> '0';
      $ErrorMessage('':'Row is already active.':error:Sel1@:'Sel1':outRow:outCol:psDsPgmNam:DctNme1@);

      // If adding make sure the entry does not exist
    ElseIf Sel = '1' and not Maintenance and found;
      $ErrorMessage('':'Error entry already exists with this name.':error:DctNme1@:'DctNme1':outRow:outCol:psDsPgmNam);
    ...
    ElseIf not (Sel in %list('1':'8')) and not found and not Maintenance;
      $ErrorMessage('':'Error - Entery does not exist.':error:DctNme1@:'DctNme1':outRow:outCol:psDsPgmNam);
    EndIf;
  EndIf;
  ...
  For sflRcd = 1 To RRN1SV;
    ...
    If %found;
      ...
      ElseIf Sel = '4' and AcvDes = 'Inactive';
        $ErrorMessage('':'Row is already inactive.':error:sel@:'Sel':outRow:outCol:psDsPgmNam);
        outRow += sflRcd - 1; // required to move to the correct SFL line
      ElseIf Sel = '13' and AcvDes = 'Active';
        $ErrorMessage('':'Row is already active.':error:sel@:'Sel':outRow:outCol:psDsPgmNam);
        outRow += sflRcd - 1; // required to move to the correct SFL line
      EndIf;
      Update SFL;
    EndIf;
...
Dcl-Proc BuildSQLStatement;
  ...
  sqlStm = '+
    ...
    From ( +
      Select Distinct +
        DCTMST.DctMstIdn Key, +
        DCTMST.DctNme DctNme, +
        DCTMST.Des Des, +
        coalesce(Acv.EnmDes,''Error'') AcvDes +
      From DCTMST +
      left join DCTVAL Acv on (Acv.DctNme,Acv.fldNme,Acv.enmVal)=(''APLDCT'',''ACVROW'',DCTMST.AcvRow) +
    )';
  ...
  If filterDs.DctNme > '';
    sqlStm += ' ' + Where + ' '
      + ' ucase(DctNme) Like uCase(''%' + %trim(filterDs.DctNme) + '%'')';
    Where = 'and';
  EndIf;
  ...
  If filterDs.Des > '';
    sqlStm += ' ' + Where + ' '
      + ' ucase(Des) Like uCase(''%' + %trim(filterDs.Des) + '%'')';
    Where = 'and';
  EndIf;
...
Dcl-Proc ProgramInitialization;
  ...
  If not %open(DCTMSTF1); // **change
    Open DCTMSTF1; // **change
  EndIf;
